...
def create_presentation(request):
    ...
    if not presentation_serializer.is_valid():
        return Response(
            presentation_serializer.errors, status=status.HTTP_400_BAD_REQUEST
        )
        ...
        responses={
            201: openapi.Schema(
                ...
                properties={
                    ...
                    "start_time": openapi.Schema(type=openapi.TYPE_STRING),
                    "end_time": openapi.Schema(type=openapi.TYPE_STRING),
                    "presentation_description": openapi.Schema(
...
def delete_presentation(request, presentationId):
    ...
    if requestUserCompany != presentationOwner:
        return Response(
            {"Forbidden": "Presentation does not belong to user"}, status=403
        )
    presentation.delete()
