...
def get_presentation(request, stallId):
    ...
    try:
        ...
        return Response(status=status.HTTP_404_NOT_FOUND)
    presentation_serializer = PresentationSerializer(presentation, fields=('stall_id','start_time','end_time', 'presentation_link', 'presentation_description'))
    return Response(presentation_serializer.data, status=status.HTTP_201_CREATED)   

@api_view(['POST', ])
def create_presentation(request):
    presentation_serializer = PresentationSerializer(data=request.data, fields=('stall_id','start_time','end_time', 'presentation_link', 'presentation_description'))
...
def edit_presentation(request):
    try:
        ...
        return Response(status=status.HTTP_404_NOT_FOUND)
    presentation_serializer = PresentationSerializer(presentation, data=request.data, fields=('stall_id','start_time','end_time', 'presentation_link', 'presentation_description'))
    if not presentation_serializer.is_valid():
        return Response(presentation_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    presentation_serializer.save()
    return Response(presentation_serializer.data, status=status.HTTP_201_CREATED)   