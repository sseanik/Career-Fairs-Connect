...
class Company(APIView):
    ...
    @swagger_auto_schema(
        request_body=openapi.Schema(
            ...
            properties={
                ...
                "company_logo_64": openapi.Schema(
                    type=openapi.TYPE_STRING, description="base64 image"
                ),
                "company_webpage_url": openapi.Schema(
        ...
        responses={
            200: openapi.Schema(
                ...
                properties={
                    ...
                    "company_description": openapi.Schema(
                        type=openapi.TYPE_STRING, description="not null"
                    ),
                    "company_logo_64": openapi.Schema(
                        type=openapi.TYPE_STRING, description="base64 image"
                    ),
    ...
    def put(self, request, companyId, format=None):
        ...
        company = get_object_or_404(Companies, pk=companyId)
        if (
            company.user_id_id != request.user.userID
        ):  # TODO user_id_id -> we need to rename this in all models
            return Response(
                "You can only update info for your company",
                status=status.HTTP_403_FORBIDDEN,
            )
        serializer = CompanySerializer(
            company,
