...
def get_all_presentations(request, eventId):
    ...
    try:
        presentations = Presentations.objects.filter(
            stall_id__in=stalls_list).values()
    except:
        return Response(status=status.HTTP_404_NOT_FOUND)
...
def get_presentation(request, stallId):
    try:
        ...
        return Response(status=status.HTTP_404_NOT_FOUND)
    presentation_serializer = PresentationSerializer(presentation, fields=(
        'stall_id', 'start_time', 'end_time', 'presentation_link', 'presentation_description', 'title', 'color', 'textColor'))
...
def create_presentation(request):
    ...
    opportunityOwner = Stalls.objects.get(
        stall_id=request.data['stall_id']).company_id.company_id
    if requestUserCompany != opportunityOwner:
        return Response({"Forbidden": "Stall does not belong to user"}, status=403)
    presentation_serializer = PresentationSerializer(data=request.data, fields=(
        'stall_id', 'start_time', 'end_time', 'presentation_link', 'presentation_description', 'title', 'color', 'textColor'))
...
def edit_presentation(request):
    ...
    if requestUserCompany != opportunityOwner:
        return Response({"Forbidden": "Stall does not belong to user"}, status=403)
    try:
        presentation = Presentations.objects.get(
            presentation_id=request.data['presentation_id'])
    except:
        return Response(status=status.HTTP_404_NOT_FOUND)
    presentation_serializer = PresentationSerializer(presentation, data=request.data, fields=(
        'presentation_id', 'stall_id', 'start_time', 'end_time', 'presentation_link', 'presentation_description', 'title', 'color', 'textColor'))
    if not presentation_serializer.is_valid():
        return Response(presentation_serializer.errors, status=status.HTTP_400_BAD_REQUEST)
    presentation_serializer.save()
    return Response(presentation_serializer.data, status=status.HTTP_201_CREATED)
